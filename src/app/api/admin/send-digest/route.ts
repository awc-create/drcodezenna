// src/app/api/admin/send-digest/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    if (to.length === 0) return NextResponse.json({ error: 'No subscribers match the selected audience' }, { status: 400 });

    // fetch selected titles to build links
